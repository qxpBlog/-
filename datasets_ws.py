...
def calculate_overlap_ratio(quad1, quad2) -> float:
    # 创建四边形
    poly1 = Polygon(quad1)
    poly2 = Polygon(quad2)
    # 计算交集
...
class PCADataset(data.Dataset):
    def __init__(self, args, datasets_folder="dataset", dataset_folder="uav/images/train"):
...
class BaseDataset(data.Dataset):
    ...
    """
    def __init__(self, args, datasets_folder="datasets", dataset_name="uav", split="train"):
        super().__init__()
        self.args = args
        ...
        with open(join(self.dataset_folder, 'database_info.csv'), 'r', encoding='utf-8') as csvfile:
            reader = csv.reader(csvfile)
            # 跳过标题行（如果CSV文件有标题行的话）
            next(reader)
            # 逐行读取数据
            self.database_utms = np.array([[(row[7], row[8]), (row[9], row[10]), (row[5], row[6]), (row[3], row[4])] for row in reader]).astype(float)
        
        with open(join(self.dataset_folder, 'queries_info.csv'), 'r', encoding='utf-8') as csvfile:
            reader = csv.reader(csvfile)
            # 跳过标题行（如果CSV文件有标题行的话）
            next(reader)
            # 逐行读取数据
            self.queries_utms = np.array([[(row[3], row[4]), (row[9], row[10]), (row[7], row[8]), (row[5], row[6])] for row in reader]).astype(float)
        # end_time = time.time()
        # print("读取坐标耗时: {:.2f}秒".format(end_time - start_time))
        # self.database_utms = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.database_paths]).astype(float)
        # self.queries_utms = np.array([(path.split("@")[1], path.split("@")[2]) for path in self.queries_paths]).astype(float)
        ...
        self.soft_positives_per_query = defaultdict(list) # 
        self.database_utm_to_idx = defaultdict(int)
        self.queries_utm_to_idx = defaultdict(int)
        # start_time = time.time()
        for i, p1 in enumerate(self.queries_utms): # 耗时问题
            nn = []
            self.queries_utm_to_idx[str(p1)] = i
            for j, p2 in enumerate(self.database_utms):
                self.database_utm_to_idx[str(p2)] = j
                if calculate_overlap_ratio(p1, p2) >= 0.1:
                    nn.append(j)
            self.soft_positives_per_query[i] = nn
        # end_time = time.time()
        # print("获取查询正样本耗时: {:.2f}秒".format(end_time - start_time)) # 耗时严重
...
class TripletsDataset(BaseDataset):
    ...
    """
    def __init__(self, args, datasets_folder="datasets", dataset_name="uav", split="train", negs_num_per_query=10):
        ...
        self.similarity_topk = args.similarity_topk
        ...
        self.query_transform = T.Compose([
                ...
                T.RandomResizedCrop(size=self.resize, scale=(1-args.random_resized_crop, 1)), # 图像大小调整
        ...
        self.hard_positives_per_query = list(self.soft_positives_per_query.values())
    ...
    # the number of positive samples fron one to similaritytopk(default: 10)
    # def __getitem__(self, index):
    #     if self.is_inference:
    #         # At inference time return the single image. This is used for caching or computing NetVLAD's clusters
    #         return super().__getitem__(index)
    #     query_index, positive_indexes, neg_indexes = torch.split(self.triplets_global_indexes[index], (1, self.similarity_topk, self.negs_num_per_query))
    #     query = self.query_transform(path_to_pil_img(self.queries_paths[query_index]))
    #     positive = [self.resized_transform(path_to_pil_img(self.database_paths[i])) for i in positive_indexes]
    #     negatives = [self.resized_transform(path_to_pil_img(self.database_paths[i])) for i in neg_indexes]
    #     images = torch.stack((query, *positive, *negatives), 0)
    #     triplets_local_indexes = torch.empty((0, 3), dtype=torch.int)

    #     for pos_num in range(len(positive_indexes)):
    #         triplets_local_indexes = torch.cat((triplets_local_indexes, torch.tensor([0, 1 + pos_num, 2]).reshape(1, 3)))
    ...
    def get_best_positive_indexes(self, args, query_index, cache, query_features, similarity_topk):
        return self.soft_positives_per_query[query_index][:similarity_topk]

    # 寻找database中query的负样本中最接近query的样本
    # 排除重叠大于10%的样本 x
    ...
    #     # Take all the positives
    #     # positives_indexes = [self.soft_positives_per_query[str(self.queries_utms[int(i)])] for i in sampled_queries_indexes]   
    #     # positives_indexes = [self.database_utm_to_idx[str(p)] for pos in positives_indexes for p in pos]
    #     positives_indexes = [int(x) for x in sampled_queries_indexes]   
    ...
    #         query_features = self.get_query_features(query_index, cache)
    #         # best_positive_index = self.get_best_positive_index(args, query_index, cache, query_features)
    #         best_positive_indexes = self.get_best_positive_indexes(args, query_index, cache, query_features, args.similarity_topk)
            
    #         # 在sampled_database_indexes中选择最难的负样本，确保没有正样本
    #         # 首先剔除重贴面积大于10%的样本
    #         # soft_positives = self.soft_positives_per_query[query_index]
    #         # soft_positives = [self.database_utm_to_idx[str(p)] for p in self.soft_positives_per_query[str(self.queries_utms[query_index])]]
    #         soft_positives = query_index
    #         neg_indexes = np.setdiff1d(sampled_database_indexes, soft_positives, assume_unique=True)
    #         neg_idx_temp = []
    #         for x in neg_indexes:
    #             if calculate_overlap_ratio(self.database_utms[query_index], self.database_utms[int(x)]) < 0.1:
    #                 neg_idx_temp.append(x)
    #         neg_indexes = np.array(neg_idx_temp)

    #         # Take all database images that are negatives and are within the sampled database images (aka database_indexes)
    #         neg_indexes = self.get_hardest_negatives_indexes(args, cache, query_features, neg_indexes)
    #         self.triplets_global_indexes.append((query_index, *best_positive_indexes, *neg_indexes))
    #         # print(query_index)
    #         # print(best_positive_index)
    #         # print(list(neg_indexes))
    #         # for i, x in enumerate(list(neg_indexes)):
    #         #     print(i,":", calculate_overlap_ratio(self.database_utms[query_index], self.database_utms[x]))
    #         # exit(0)
    #     # self.triplets_global_indexes is a tensor of shape [1000, 12]
    #     self.triplets_global_indexes = torch.tensor(self.triplets_global_indexes)
    ...
    def compute_triplets_partial(self, args, model):
        ...
        # print(list(sampled_queries_indexes + self.database_num))
        # print(database_indexes + list(sampled_queries_indexes + self.database_num))
        # print(Subset(self, database_indexes + list(sampled_queries_indexes + self.database_num)))
        # exit(0)
        subset_ds = Subset(self, database_indexes + list(sampled_queries_indexes + self.database_num))
        cache = self.compute_cache(args, model, subset_ds, cache_shape=(len(self), args.features_dim))
