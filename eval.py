...
if args.off_the_shelf.startswith("radenovic") or args.off_the_shelf.startswith("naver"):
    if args.off_the_shelf.startswith("radenovic"):
        ...
        state_dict = load_url(url, student_model_dir=join("data", "off_the_shelf_nets"))
    else:
    ...
    student_model_keys = student_model.state_dict().keys()
    renamed_state_dict = {k: v for k, v in zip(student_model_keys, state_dict.values())}
    student_model.load_state_dict(renamed_state_dict)
elif args.resume is not None:
    logging.info(f"Resuming student_model from {args.resume}")
    student_model = util.resume_student_model(args, student_model)
...
if args.pca_dim is None:
    ...
    pca = util.compute_pca(args, student_model, args.pca_dataset_folder, full_features_dim)
...
######################################### TEST on TEST SET #########################################
recalls, recalls_str = test.test(args, test_ds, student_model, teacher_model, args.test_method, pca)
