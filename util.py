...
def save_checkpoint(args, state, is_best, filename, modelname=''):
    model_path = join(args.save_dir, filename)
    torch.save(state, model_path)
    if is_best:
        if modelname is 'teacher':
            shutil.copyfile(model_path, join(args.save_dir, "teacher_best_model.pth"))
        elif modelname is 'student':
            shutil.copyfile(model_path, join(args.save_dir, "student_best_model.pth"))
        else:
...
def resume_train_student(args, model, optimizer=None, strict=False):
    """Load model, optimizer, and other training parameters"""
    logging.debug(f"Loading checkpoint: {args.resume_student}")
    checkpoint = torch.load(args.resume_student)
    start_epoch_num = checkpoint["epoch_num"]
    model.load_state_dict(checkpoint["student_model_state_dict"], strict=strict)
    if optimizer:
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
    best_r5 = checkpoint["best_r5"]
    not_improved_num = checkpoint["not_improved_num"]
    logging.debug(f"Loaded checkpoint: start_epoch_num = {start_epoch_num}, "
                  f"current_best_R@5 = {best_r5:.1f}")
    if args.resume_student.endswith("last_model.pth"):  # Copy best model to current save_dir
        shutil.copy(args.resume_student.replace("last_model.pth", "best_model.pth"), args.save_dir)
    return model, optimizer, best_r5, start_epoch_num, not_improved_num

def resume_train_teacher(args, model, optimizer=None, strict=False):
    """Load model, optimizer, and other training parameters"""
    logging.debug(f"Loading checkpoint: {args.resume_teacher}")
    checkpoint = torch.load(args.resume_teacher)
    start_epoch_num = checkpoint["epoch_num"]
    model.load_state_dict(checkpoint["teacher_model_state_dict"], strict=strict)
    if optimizer:
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
    best_r5 = checkpoint["best_r5"]
    not_improved_num = checkpoint["not_improved_num"]
    logging.debug(f"Loaded checkpoint: start_epoch_num = {start_epoch_num}, "
                  f"current_best_R@5 = {best_r5:.1f}")
    if args.resume_teacher.endswith("last_model.pth"):  # Copy best model to current save_dir
        shutil.copy(args.resume_teacher.replace("last_model.pth", "best_model.pth"), args.save_dir)
    return model, optimizer, best_r5, start_epoch_num, not_improved_num
