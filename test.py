...
from datasets_ws import calculate_overlap_ratio
...
def test(args, eval_ds, student_model, teacher_model, test_method="hard_resize", pca=None):
    """Compute features of the given dataset and compute the recalls."""
    ...
    if args.efficient_ram_testing:
        return test_efficient_ram_usage(args, eval_ds, student_model, test_method)
    ...
    with torch.no_grad():
        ...
        for inputs, indices in tqdm(database_dataloader, ncols=100):
            features = teacher_model(inputs.to(args.device)) # teacher model
            features = features.cpu().numpy()
            if pca is not None:
                features = pca.transform(features)
            # features = features.reshape(features.shape[0], -1)[:, :614016] # transVPR
            all_features[indices.numpy(), :] = features
        ...
        queries_dataloader = DataLoader(dataset=queries_subset_ds, num_workers=args.num_workers,
                                        batch_size=queries_infer_batch_size, pin_memory=(args.device == "cuda"))
        for inputs, indices in tqdm(queries_dataloader, ncols=100):
            features = student_model(inputs.to(args.device))
            features = features.cpu().numpy()
            if pca is not None:
                features = pca.transform(features)
            # features = features.reshape(features.shape[0], -1)[:, :614016] # transVPR
    ...
    for query_index, pred in enumerate(predictions):
        for i, n in enumerate(args.recall_values):
            for j in range(n):
                if calculate_overlap_ratio(eval_ds.database_utms[pred[j]], eval_ds.database_utms[query_index]) >= args.val_positive_area_threshold:
                    recalls[i] += 1
                    break  
    # Divide by the number of queries*100, so the recalls are in percentages
    recalls = recalls / eval_ds.queries_num * 100
...
#         for inputs, indices in tqdm(database_dataloader, ncols=100):
#             features = model(inputs.to(args.device)) # teacher model
#             features = features.cpu().numpy()
#             if pca is not None:
#                 features = pca.transform(features)
#             all_features[indices.numpy(), :] = features
